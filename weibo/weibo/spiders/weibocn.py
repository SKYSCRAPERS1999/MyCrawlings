...
class WeibocnSpider(Spider):
    ...
    regex = re.compile('(?:谈股|说股|讲股|论股|基金|看股|港股|美股|理财师|A股|经济政策|短线|炒股|荐股|盈亏|炒股|股利|股海|股民|股票|个股|做空|诱多|诱空|踏空|长空|短空|长多|短多|死多|翻多|翻空|多杀多|扎空|空仓|建仓|满仓|斩仓|减仓|加仓|重仓|清仓|套牢|补仓|仓位|庄家|震仓|追涨|杀跌|止盈|抄底|逃顶|盘整|反弹|回档|坐庄|吸筹|对敲|打压|洗盘|散户|中户|坐轿|主力|抬轿|筹码|抢帽子|多头陷阱|空头陷阱|护盘|跳空|开盘价|收盘价|最高价|最低价|成交量|放量|缩量|热门股|冷门股|白马股|黑马股|龙头股|阴跌|换手率|现手|平开|低开|高开|内盘|外盘|均价|浮筹|市盈率|含权|回购|基本面分析|量比|每股|收益|市净率|探底|填权|停牌|退市)')    
    nregex = re.compile('(?:公司|企业|CEO|基金会|大学|学院)')
    
    finish_ID = set(start_users)  # 记录已爬的微博ID
    garbage_ID = set()
    ...
    def validify_user(self, user_info):
        ...
        if re.search(self.regex, user_info.get('description')) == None:
            if user_info.get('id') != None:
                self.garbage_ID.add(user_info.get('id'))
            return False
    ...
    def parse_user(self, response):
        ...
        if result.get('data').get('userInfo'):
            ...
            self.finish_ID.add(uid)
            
            if len(self.finish_ID) % 10 == 0:
                print ('finish: ', len(self.finish_ID))
                print ('garbage: ', len(self.garbage_ID), '\n')
            # 微博
